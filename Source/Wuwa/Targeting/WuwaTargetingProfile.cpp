...
bool UWuwaTargetingProfile::IsRuntimeValid() const
{
	...
	const bool bValidRanges =
	    FMath::IsFinite(SearchRadius) && SearchRadius > 0.f && FMath::IsFinite(MaxAcquireAngleDegrees) &&
	    MaxAcquireAngleDegrees > 0.f && MaxAcquireAngleDegrees <= 180.f && FMath::IsFinite(HardLockReleaseDistance) &&
	    HardLockReleaseDistance >= SearchRadius && FMath::IsFinite(OcclusionGraceTime) && OcclusionGraceTime >= 0.f &&
	...
	const bool bValidScore = FMath::IsFinite(DistanceWeight) && DistanceWeight >= 0.f &&
	...
	const bool bValidSwitch = FMath::IsFinite(SwitchMinimumHorizontalDelta) && SwitchMinimumHorizontalDelta >= 0.f &&
...
EDataValidationResult UWuwaTargetingProfile::IsDataValid(FDataValidationContext& Context) const
{
	...
	Check(FMath::IsFinite(DistanceWeight) && DistanceWeight >= 0.f, TEXT("DistanceWeight 必须是有限非负数"));
	...
	const ECollisionChannel TraceChannel = VisibilityTraceChannel.GetValue();
	Check(TraceChannel >= ECC_WorldStatic && TraceChannel < ECC_MAX, TEXT("VisibilityTraceChannel 必须是有效碰撞通道"));
