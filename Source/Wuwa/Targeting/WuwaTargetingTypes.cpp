...
bool FWuwaTargetingNetworkCommand::IsValid() const
{
	const bool bKnownKind = Kind == EWuwaTargetingNetworkCommandKind::EnterHardLock ||
	                        Kind == EWuwaTargetingNetworkCommandKind::SwitchHardTarget ||
	                        Kind == EWuwaTargetingNetworkCommandKind::ClearHardLock;
	if (!bKnownKind || CommandSequence <= 0)
	{
		return false;
	}

	if (Kind == EWuwaTargetingNetworkCommandKind::ClearHardLock)
	{
		return RequestedTarget == nullptr;
	}

	return ::IsValid(RequestedTarget) && RequestedTarget->GetIsReplicated();
}

bool FWuwaReplicatedTargetingAuthorityState::IsValid() const
{
	const bool bKnownMode =
	    Mode == EWuwaTargetingMode::None || Mode == EWuwaTargetingMode::Soft || Mode == EWuwaTargetingMode::Hard;
	const bool bValidResult = (bAccepted && FailureReason == EWuwaTargetingFailureReason::None) ||
	                          (!bAccepted && FailureReason != EWuwaTargetingFailureReason::None);
	if (!bKnownMode || !bValidResult || ProcessedCommandSequence < 0 || ContextSequence < 0)
	{
		return false;
	}

	if (Mode == EWuwaTargetingMode::None)
	{
		return TargetActor == nullptr;
	}

	return ::IsValid(TargetActor) && TargetActor->GetIsReplicated();
}

bool FWuwaTargetScoreBreakdown::IsFinite() const
{
	return FMath::IsFinite(Distance) && Distance >= 0.f && FMath::IsFinite(ViewAngleDegrees) &&
	       ViewAngleDegrees >= 0.f && FMath::IsFinite(ViewSpaceHorizontal) && FMath::IsFinite(ViewSpaceVertical) &&
	       ...
	       FMath::IsFinite(RetentionBonus) && RetentionBonus >= 0.f && FMath::IsFinite(TotalScore) && TotalScore >= 0.f;
}
...
bool WuwaTargetingRules::CalculateScore(const FWuwaTargetScoreInput& Input, FWuwaTargetScoreBreakdown& OutScore)
{
	...
	const bool bFiniteInput = FMath::IsFinite(Input.Distance) && FMath::IsFinite(Input.SearchRadius) &&
	                          FMath::IsFinite(Input.ViewAngleDegrees) &&
	                          FMath::IsFinite(Input.MaxAcquireAngleDegrees) &&
	                          FMath::IsFinite(Input.ViewSpaceHorizontal) && FMath::IsFinite(Input.ViewSpaceVertical) &&
	                          FMath::IsFinite(Input.DistanceWeight) && FMath::IsFinite(Input.ViewAlignmentWeight) &&
	                          FMath::IsFinite(Input.RetentionBonus);

	const float TotalWeight = Input.DistanceWeight + Input.ViewAlignmentWeight;

	const bool bValidRange = Input.Distance >= 0.f && Input.SearchRadius > 0.f &&
	                         Input.Distance <= Input.SearchRadius && Input.ViewAngleDegrees >= 0.f &&
	                         Input.MaxAcquireAngleDegrees > 0.f &&
	                         Input.ViewAngleDegrees <= Input.MaxAcquireAngleDegrees && Input.DistanceWeight >= 0.f &&
	                         Input.ViewAlignmentWeight >= 0.f && FMath::IsFinite(TotalWeight) && TotalWeight > 0.f &&
	...
	OutScore.TotalScore = Input.DistanceWeight * OutScore.DistanceScore +
	                      Input.ViewAlignmentWeight * OutScore.ViewAlignmentScore + Input.RetentionBonus;
...
int32 WuwaTargetingRules::SelectBestCandidateIndex(const TArray<FWuwaTargetScoreBreakdown>& CandidateScores)
{
	...
	for (int32 Index = 0; Index < CandidateScores.Num(); ++Index)
	{
		...
		const bool bBetterAngle =
		    bEqualTotal && Candidate.ViewAngleDegrees < Best.ViewAngleDegrees - UE_KINDA_SMALL_NUMBER;
		const bool bEqualAngle =
		    FMath::IsNearlyEqual(Candidate.ViewAngleDegrees, Best.ViewAngleDegrees, UE_KINDA_SMALL_NUMBER);
...
int32 WuwaTargetingRules::SelectSwitchCandidateIndex(const TArray<FWuwaTargetScoreBreakdown>& CandidateScores,
...
{
	const bool bValidParameters = CandidateScores.IsValidIndex(CurrentCandidateIndex) && FMath::IsFinite(Direction) &&
	                              !FMath::IsNearlyZero(Direction) && FMath::IsFinite(MinimumHorizontalDelta) &&
	                              MinimumHorizontalDelta >= 0.f && FMath::IsFinite(VerticalPenaltyWeight) &&
	                              VerticalPenaltyWeight >= 0.f && FMath::IsFinite(DistancePenaltyWeight) &&
	                              DistancePenaltyWeight >= 0.f;
	...
	for (int32 Index = 0; Index < CandidateScores.Num(); ++Index)
	{
		...
		const float SwitchCost = DirectionalHorizontalDelta + VerticalPenaltyWeight * VerticalDelta +
		...
		const bool bHigherTargetScore =
		    bEqualCost && (BestIndex == INDEX_NONE ||
		                   Candidate.TotalScore > CandidateScores[BestIndex].TotalScore + UE_KINDA_SMALL_NUMBER);
	...
	for (int32 Index = 0; Index < CandidateScores.Num(); ++Index)
	{
		...
		const bool bEqualDepth =
		    FMath::IsNearlyEqual(DirectionalDistanceDelta, BestDirectionalDistanceDelta, UE_KINDA_SMALL_NUMBER);
		const bool bSmallerVerticalDelta =
		    bEqualDepth && VerticalDelta < BestOverlapVerticalDelta - UE_KINDA_SMALL_NUMBER;
		const bool bEqualVerticalDelta =
		    bEqualDepth && FMath::IsNearlyEqual(VerticalDelta, BestOverlapVerticalDelta, UE_KINDA_SMALL_NUMBER);
		const bool bHigherTargetScore =
		    bEqualVerticalDelta &&
