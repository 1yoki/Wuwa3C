...
namespace
{
    ...
    bool BuildHorizontalCombatAxis(
        const FVector &PlayerPoint,
        const FVector &TargetPoint,
        FVector &OutCombatAxis)
    {
        OutCombatAxis = FVector::ZeroVector;

        if (!IsFiniteVector(PlayerPoint) || !IsFiniteVector(TargetPoint))
        {
            return false;
        }
        ...
        return !OutCombatAxis.IsNearlyZero() && IsFiniteVector(OutCombatAxis);
    }
}

bool WuwaCameraFramingRules::CalculateHorizontalCombatAxisYaw(
    const FVector &PlayerPoint,
    const FVector &TargetPoint,
    float &OutYawDegrees)
{
...
bool WuwaCameraFramingRules::CalculateCameraCombatAxisProjection(
    const FVector &PlayerPoint,
    const FVector &TargetPoint,
                                                             ...
                                                             float &OutCorrectedAngleDegrees)
{
    ...
    const bool bHasFiniteInput =
        FMath::IsFinite(Input.CurrentAngleDegrees) &&
        FMath::IsFinite(Input.DesiredAngleDegrees) &&
        FMath::IsFinite(Input.DeadZoneHalfAngleDegrees) &&
        FMath::IsFinite(Input.MaxCorrectionSpeedDegrees) &&
        FMath::IsFinite(Input.DeltaTime) &&
        FMath::IsFinite(Input.AuthorityAlpha);

    const bool bHasValidRange =
        Input.DeadZoneHalfAngleDegrees >= 0.0f &&
        Input.DeadZoneHalfAngleDegrees < 180.0f &&
        Input.MaxCorrectionSpeedDegrees > 0.0f &&
        Input.DeltaTime >= 0.0f &&
        Input.AuthorityAlpha >= 0.0f &&
        Input.AuthorityAlpha <= 1.0f;
