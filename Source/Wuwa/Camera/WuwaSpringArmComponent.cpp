...
bool UWuwaSpringArmComponent::ConfigureCollision(const float InProbeSize,
                                                 ECollisionChannel InProbeChannel,
                                                 const float InRecoveryInterpSpeed)
{
	if (!FMath::IsFinite(InProbeSize) || InProbeSize <= 0.f || !IsValidCollisionChannel(InProbeChannel) ||
	    !FMath::IsFinite(InRecoveryInterpSpeed) || InRecoveryInterpSpeed <= 0.f)
	{
		return false;
	}
...
FVector UWuwaSpringArmComponent::BlendLocations(const FVector& DesiredArmLocation,
                                                const FVector& TraceHitLocation,
                                                const bool bHitSomething,
                                                const float DeltaTime)
{
	const FVector NativeLocation =
	...
	if (DesiredArmLocation.ContainsNaN() || NativeLocation.ContainsNaN() || ArmOrigin.ContainsNaN() ||
	    !FMath::IsFinite(DeltaTime) || DeltaTime < 0.f || !FMath::IsFinite(CollisionRecoveryInterpSpeed) ||
	...
	else if (NativeDistance > CurrentResolvedDistance + RecoveryDistanceTolerance)
	{
		...
		CurrentResolvedDistance =
		    FMath::FInterpTo(CurrentResolvedDistance, NativeDistance, DeltaTime, CollisionRecoveryInterpSpeed);
	}
	else
	{
		// 误差范围内不更新距离，避免抖动。
