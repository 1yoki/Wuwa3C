...
namespace
{
    bool IsValidCameraCollisionChannel(
...
const FWuwaCameraModeConfig *UWuwaCameraProfile::FindModeConfig(const FGameplayTag &ModeTag) const
{
    ...
    return ModeConfigs.FindByPredicate([&ModeTag](const FWuwaCameraModeConfig &Config)
...
bool UWuwaCameraProfile::IsRuntimeValid() const
{
    ...
    for (const FWuwaCameraModeConfig &Config : ModeConfigs)
    {
        ...
        const bool bSupportedMode = Config.ModeTag == WuwaGameplayTags::Camera_Exploration ||
                                    Config.ModeTag == WuwaGameplayTags::Camera_LockOn;
    ...
    const bool bLockOnTargetCompositionEnabled = LockOn->TargetPivotWeight > 0.0f &&
                                                 LockOn->MaxTargetPivotOffset > 0.0f &&
                                                 LockOn->TargetDistanceArmScale > 0.0f &&
                                                 LockOn->LockOnYawDeadZoneHalfAngle > 0.0f &&
                                                 LockOn->LockOnPitchDeadZoneHalfAngle > 0.0f &&
                                                 LockOn->LockOnMaxYawCorrectionSpeed > 0.0f &&
                                                 LockOn->LockOnMaxPitchCorrectionSpeed > 0.0f;

    const ECollisionChannel CollisionChannel = ProbeChannel.GetValue();

    const bool bValidCollision = FMath::IsFinite(ProbeSize) &&
                                 ProbeSize > 0.f &&
                                 IsValidCameraCollisionChannel(CollisionChannel) &&
                                 FMath::IsFinite(CollisionRecoveryInterpSpeed) &&
                                 CollisionRecoveryInterpSpeed > 0.f;
...
EDataValidationResult UWuwaCameraProfile::IsDataValid(
    FDataValidationContext &Context) const
{
    ...
    Check(
        ModeConfigs.Num() == 2,
        TEXT("ModeConfigs 必须恰好包含 Exploration 和 LockOn 两项"));

    TSet<FGameplayTag> SeenModeTags;

    for (int32 Index = 0; Index < ModeConfigs.Num(); ++Index)
    {
        const FWuwaCameraModeConfig &Config =
            ModeConfigs[Index];
        ...
        Check(
            Config.ModeTag.IsValid(),
            Prefix + TEXT(".ModeTag 必须有效"));

        const bool bSupportedMode =
            Config.ModeTag == WuwaGameplayTags::Camera_Exploration ||
            Config.ModeTag == WuwaGameplayTags::Camera_LockOn;

        Check(
            bSupportedMode,
            Prefix + TEXT(".ModeTag 只能是 Camera.Exploration 或 Camera.LockOn"));

        if (Config.ModeTag.IsValid())
        {
            Check(
                !SeenModeTags.Contains(Config.ModeTag),
                Prefix + TEXT(".ModeTag 与另一项重复"));
        ...
        Check(
            Config.Priority >= 0,
            Prefix + TEXT(".Priority 不能为负数"));

        Check(
            FMath::IsFinite(Config.FieldOfView) &&
                Config.FieldOfView >= 5.f &&
                Config.FieldOfView <= 170.f,
            Prefix + TEXT(".FieldOfView 必须是 [5, 170] 内的有限值"));

        Check(
            FMath::IsFinite(Config.TargetArmLength) &&
                Config.TargetArmLength > 0.f,
            Prefix + TEXT(".TargetArmLength 必须是有限正数"));

        const bool bFiniteTargetOffset =
            FMath::IsFinite(Config.TargetOffset.X) &&
            FMath::IsFinite(Config.TargetOffset.Y) &&
            FMath::IsFinite(Config.TargetOffset.Z);

        Check(
            bFiniteTargetOffset,
            Prefix + TEXT(".TargetOffset 必须全部为有限值"));

        const bool bFiniteSocketOffset =
            FMath::IsFinite(Config.SocketOffset.X) &&
            FMath::IsFinite(Config.SocketOffset.Y) &&
            FMath::IsFinite(Config.SocketOffset.Z);

        Check(
            bFiniteSocketOffset,
            Prefix + TEXT(".SocketOffset 必须全部为有限值"));

        Check(
            FMath::IsFinite(Config.BlendInTime) &&
                Config.BlendInTime >= 0.f,
            Prefix + TEXT(".BlendInTime 必须是有限非负数"));

        Check(
            FMath::IsFinite(Config.BlendOutTime) &&
                Config.BlendOutTime >= 0.f,
            Prefix + TEXT(".BlendOutTime 必须是有限非负数"));

        Check(
            FMath::IsFinite(Config.TargetPivotWeight) &&
                Config.TargetPivotWeight >= 0.f &&
                Config.TargetPivotWeight <= 1.f,
            Prefix + TEXT(".TargetPivotWeight 必须在 [0, 1] 范围内"));

        Check(
            FMath::IsFinite(Config.MaxTargetPivotOffset) &&
                Config.MaxTargetPivotOffset >= 0.f,
            Prefix + TEXT(".MaxTargetPivotOffset 必须是有限非负数"));

        Check(
            FMath::IsFinite(Config.TargetDistanceArmScale) &&
                Config.TargetDistanceArmScale >= 0.f,
            Prefix + TEXT(".TargetDistanceArmScale 必须是有限非负数"));

        Check(
            FMath::IsFinite(Config.LockOnYawDeadZoneHalfAngle) &&
                Config.LockOnYawDeadZoneHalfAngle >= 0.0f &&
                Config.LockOnYawDeadZoneHalfAngle < 180.0f,
            Prefix + TEXT(".LockOnYawDeadZoneHalfAngle 必须位于 [0, 180) 度"));

        Check(
            FMath::IsFinite(Config.LockOnPitchDeadZoneHalfAngle) &&
                Config.LockOnPitchDeadZoneHalfAngle >= 0.0f &&
                Config.LockOnPitchDeadZoneHalfAngle < 90.0f,
            Prefix + TEXT(".LockOnPitchDeadZoneHalfAngle 必须位于 [0, 90) 度"));

        Check(
            FMath::IsFinite(Config.LockOnMaxYawCorrectionSpeed) &&
                Config.LockOnMaxYawCorrectionSpeed >= 0.0f,
            Prefix + TEXT(".LockOnMaxYawCorrectionSpeed 必须是有限非负数"));

        Check(
            FMath::IsFinite(Config.LockOnMaxPitchCorrectionSpeed) &&
                Config.LockOnMaxPitchCorrectionSpeed >= 0.0f,
            Prefix + TEXT(".LockOnMaxPitchCorrectionSpeed 必须是有限非负数"));

        Check(
            FMath::IsFinite(Config.MinTargetArmLength) &&
                Config.MinTargetArmLength > 0.f,
            Prefix + TEXT(".MinTargetArmLength 必须是有限正数"));

        Check(
            FMath::IsFinite(Config.MaxTargetArmLength) &&
                Config.MaxTargetArmLength >= Config.MinTargetArmLength,
            Prefix + TEXT(".MaxTargetArmLength 必须是有限值且不能小于最小臂长"));

        Check(
            Config.TargetArmLength >= Config.MinTargetArmLength &&
                Config.TargetArmLength <= Config.MaxTargetArmLength,
            Prefix + TEXT(".TargetArmLength 必须位于 Min/Max TargetArmLength 范围内"));
    }

    const FWuwaCameraModeConfig *Exploration =
        FindModeConfig(WuwaGameplayTags::Camera_Exploration);
    const FWuwaCameraModeConfig *LockOn =
        FindModeConfig(WuwaGameplayTags::Camera_LockOn);

    Check(
        Exploration != nullptr,
        TEXT("ModeConfigs 缺少 Camera.Exploration"));

    Check(
        LockOn != nullptr,
        TEXT("ModeConfigs 缺少 Camera.LockOn"));

    if (Exploration != nullptr)
    {
        Check(
            FMath::IsNearlyZero(Exploration->TargetPivotWeight) &&
                FMath::IsNearlyZero(Exploration->MaxTargetPivotOffset) &&
                FMath::IsNearlyZero(Exploration->TargetDistanceArmScale) &&
                FMath::IsNearlyZero(
                    Exploration->LockOnYawDeadZoneHalfAngle) &&
                FMath::IsNearlyZero(
                    Exploration->LockOnPitchDeadZoneHalfAngle) &&
                FMath::IsNearlyZero(
                    Exploration->LockOnMaxYawCorrectionSpeed) &&
                FMath::IsNearlyZero(
                    Exploration->LockOnMaxPitchCorrectionSpeed),
    ...
    {
        Check(
            LockOn->TargetPivotWeight > 0.f,
            TEXT("LockOn.TargetPivotWeight 必须大于 0"));

        Check(
            LockOn->MaxTargetPivotOffset > 0.f,
            TEXT("LockOn.MaxTargetPivotOffset 必须大于 0"));

        Check(
            LockOn->TargetDistanceArmScale > 0.f,
            TEXT("LockOn.TargetDistanceArmScale 必须大于 0"));
        Check(
            LockOn->LockOnYawDeadZoneHalfAngle > 0.0f,
            TEXT("LockOn.LockOnYawDeadZoneHalfAngle 必须大于 0"));

        Check(
            LockOn->LockOnPitchDeadZoneHalfAngle > 0.0f,
            TEXT("LockOn.LockOnPitchDeadZoneHalfAngle 必须大于 0"));

        Check(
            LockOn->LockOnMaxYawCorrectionSpeed > 0.0f,
            TEXT("LockOn.LockOnMaxYawCorrectionSpeed 必须大于 0"));

        Check(
            LockOn->LockOnMaxPitchCorrectionSpeed > 0.0f,
            TEXT("LockOn.LockOnMaxPitchCorrectionSpeed 必须大于 0"));
    }

    if (Exploration != nullptr && LockOn != nullptr)
    {
        Check(
            LockOn->Priority > Exploration->Priority,
            TEXT("LockOn.Priority 必须高于 Exploration.Priority"));
    }

    Check(
        FMath::IsFinite(ProbeSize) &&
            ProbeSize > 0.f,
        TEXT("ProbeSize 必须是有限正数"));

    Check(
        IsValidCameraCollisionChannel(ProbeChannel.GetValue()),
        TEXT("ProbeChannel 必须是有效碰撞通道"));
