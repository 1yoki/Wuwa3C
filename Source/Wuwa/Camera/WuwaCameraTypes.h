...
struct WUWA_API FWuwaCameraModeRequestHandle
{
    ...
    FWuwaCameraModeRequestHandle(const FGuid &InId, const FGameplayTag &InModeTag)
        : Id(InId), ModeTag(InModeTag)
    {
    }
...
struct WUWA_API FWuwaCameraRigState
{
    ...
    bool IsFinite() const
    {
        ...
        return FMath::IsFinite(FieldOfView) && FMath::IsFinite(TargetArmLength) &&
               IsFiniteVector(TargetOffset) && IsFiniteVector(SocketOffset);
    }
};
...
struct WUWA_API FWuwaCameraModeConfig
{
    ...
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Camera Mode|Rig", meta = (ClampMin = "5.0", ClampMax = "170.0", Units = "deg"))
    float FieldOfView = 90.f;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Camera Mode|LockOn", meta = (ClampMin = "0.0", ClampMax = "1.0"))
     ...
     */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Camera Mode|LockOn|Framing", meta = (ClampMin = "0.0", ClampMax = "179.0", Units = "deg"))
     ...
     */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Camera Mode|LockOn|Framing", meta = (ClampMin = "0.0", ClampMax = "89.0", Units = "deg"))
    float LockOnPitchDeadZoneHalfAngle = 0.0f;

    // LockOn 超出水平安全区后允许的最大 Yaw 修正速度。
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Camera Mode|LockOn|Framing", meta = (ClampMin = "0.0", Units = "deg/s"))
    ...
    bool IsRuntimeValid() const
    {
        ...
        return ModeTag.IsValid() &&
               ...
               FMath::IsFinite(BlendInTime) &&
               BlendInTime >= 0.f &&
               FMath::IsFinite(BlendOutTime) &&
               BlendOutTime >= 0.f &&
               FMath::IsFinite(TargetPivotWeight) &&
               TargetPivotWeight >= 0.f &&
               TargetPivotWeight <= 1.f &&
               FMath::IsFinite(MaxTargetPivotOffset) &&
               MaxTargetPivotOffset >= 0.f &&
               FMath::IsFinite(TargetDistanceArmScale) &&
               TargetDistanceArmScale >= 0.f &&
               FMath::IsFinite(LockOnYawDeadZoneHalfAngle) &&
               LockOnYawDeadZoneHalfAngle >= 0.0f &&
               LockOnYawDeadZoneHalfAngle < 180.0f &&
               FMath::IsFinite(LockOnPitchDeadZoneHalfAngle) &&
               LockOnPitchDeadZoneHalfAngle >= 0.0f &&
               LockOnPitchDeadZoneHalfAngle < 90.0f &&
               FMath::IsFinite(LockOnMaxYawCorrectionSpeed) &&
               LockOnMaxYawCorrectionSpeed >= 0.0f &&
               FMath::IsFinite(LockOnMaxPitchCorrectionSpeed) &&
               LockOnMaxPitchCorrectionSpeed >= 0.0f &&
               FMath::IsFinite(MinTargetArmLength) &&
               MinTargetArmLength > 0.f &&
               FMath::IsFinite(MaxTargetArmLength) &&
               MaxTargetArmLength >= MinTargetArmLength &&
               TargetArmLength >= MinTargetArmLength &&
