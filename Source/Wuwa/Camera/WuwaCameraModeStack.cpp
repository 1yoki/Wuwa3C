...
{
	if (!bInitialized || !ModeTag.IsValid() || ModeTag == FallbackModeTag || !RuntimeConfigs.Contains(ModeTag) ||
	    !IsValid(Source))
	{
...
const FWuwaCameraModeStack::FRuntimeRequest* FWuwaCameraModeStack::FindWinningRequest(FGuid* OutHandleId) const
{
	...
	for (const TPair<FGuid, FRuntimeRequest>& Pair : Requests)
	{
		...
		const bool bNewerAtSamePriority = WinningRequest != nullptr && Config->Priority == WinningPriority &&
